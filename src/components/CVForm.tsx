...
export default function CVForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cvFile) {
      alert('Por favor, selecciona un archivo de CV.');
      return;
    }

    const formData = new FormData();
    formData.append('cv', cvFile);
    formData.append('profesor_id', '1'); // Aquí debes obtener el ID del profesor
    formData.append('cvData', JSON.stringify(cv));

    try {
      const response = await fetch('https://backend-maestros.onrender.com/upload-cv', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        console.log('CV subido correctamente:', result);
        alert('CV subido correctamente.');
      } else {
        console.error('Error al subir el CV:', response.statusText);
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Campo para subir el archivo del CV */}
            <div className="mt-4">
              <label className="block text-sm font-medium text-gray-700">Subir CV (PDF)</label>
              <input
                type="file"
